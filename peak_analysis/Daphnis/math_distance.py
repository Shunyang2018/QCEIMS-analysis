...
def weighted_entropy_distance(p, q):
    ...
    entropy_p = scipy.stats.entropy(p)
    if entropy_p < ENTROPY_END:
        weight = 1 - (1 - entropy_p / ENTROPY_END) * (1 - WEIGHT_START)
        p = np.power(p, weight)
        p = p / sum(p)

    entropy_q = scipy.stats.entropy(q)
    if entropy_q < ENTROPY_END:
        weight = 1 - (1 - entropy_q / ENTROPY_END) * (1 - WEIGHT_START)
        q = np.power(q, weight)
        q = q / sum(q)
...
def entropy_reverse_distance(p, q):
