...
#the design is based on the notion that the empirical spectrum is y and the 
#insilico spectrum is x
def tanimoto_sim(x,y,how):

    temp_numerator=0
    temp_denominator=0
    temp_similarity=0

    if how=='overprediction':
        temp_numerator=dot_product(x,x)-dot_product(x,y)
        temp_denominator=((np.linalg.norm(x)**(2))+(np.linalg.norm(y)**(2))-dot_product(x,y))
        temp_similarity=temp_numerator/temp_denominator

    elif how=='underprediction':
        temp_numerator=dot_product(y,y)-dot_product(x,y)
        temp_denominator=((np.linalg.norm(x)**(2))+(np.linalg.norm(y)**(2))-dot_product(x,y))
        temp_similarity=temp_numerator/temp_denominator

    elif how=='intersection':
        temp_numerator=dot_product(x,y)
...
#intentionally not named dot_sim so things dont erroneously call this
def dot_product(x,y):
    temp_result=np.dot(x,y)
    return temp_result


def weighted_dot_sim(x,y):
    '''
